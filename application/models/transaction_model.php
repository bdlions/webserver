...
class Transaction_model extends Ion_auth_model 
{
    ...
    public function get_agents_current_balance()
    {
        ...
        return $this->db->select($this->tables['user_transactions'].'.user_id, first_name, last_name, sum(balance_in) - sum(balance_out) as current_balance')
            ->from($this->tables['user_transactions'])
            ->join($this->tables['users_groups'], $this->tables['users_groups'] . '.user_id=' . $this->tables['user_transactions'] . '.user_id')
