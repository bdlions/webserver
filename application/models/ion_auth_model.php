...
class Ion_auth_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        $this->add_user_commission($id, $additional_data);
        if(isset($additional_data['agent_user_id']))
        {
            $this->map_apent_subagent($additional_data['agent_user_id'], $id);
        }
    ...
    public function add_user_commission($user_id, $additional_data)
    {
        $data = array(
            'user_id' => $user_id
        );
        $commission_data = array_merge($this->_filter_data($this->tables['users_commissions'], $additional_data),$data);
        $this->db->insert($this->tables['users_commissions'], $commission_data);
    }
    
    public function map_apent_subagent($agent_user_id, $subagent_user_id)
    {
        $data = array(
            'subagent_user_id' => $subagent_user_id,
            'agent_user_id' => $agent_user_id
        );
        $this->db->insert($this->tables['subagents'], $data);
    }
}
