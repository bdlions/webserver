...
class Template {
    ...
    function getUserTemplate($group_name){
        if($group_name == AGENT){
            $this->selected_user_group = AGENT;
            return AGENT_LOGIN_SUCCESS_TEMPLATE;
        }
        else if($group_name == SUBAGENT){
            $this->selected_user_group = SUBAGENT;
            return SUBAGENT_LOGIN_SUCCESS_TEMPLATE;
        }
