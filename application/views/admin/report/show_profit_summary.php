<script>
    $(function() {
        $("#admin_list_profits").change(function() {
            var user_id = $('#admin_list_profits').val();
            if(user_id > 0)
            {
        ...
        $("#agent_list_profits").change(function() {
            var agent_user_id = $('#agent_list_profits').val();
            if(agent_user_id > 0)
            {
                $('#admin_list_profits').val('');    
                get_user_profit_list(agent_user_id);
                $.ajax({
                    dataType: 'json',
                    type: "POST",
                    ...
                    success: function(data) {
                        $('#subagent_list_profits').html(tmpl('tlmp_subagent_list_profit', data.subagent_list));
                    }
                });
            }
        });
...
<script type="text/x-tmpl" id="tlmp_profit_list">
    ...
    {% while(profit_info){ %}
        <tr>
            ...
            <td>{%= profit_info.admin_profit %}</td>
...
<script type="text/x-tmpl" id="tlmp_subagent_list_profit">
            ...
            <div class="box-content">
                <div class="row form-group">
                    <div class="col-md-offset-1 col-md-3 pull-left">
                        <label>
                            <?php echo form_dropdown('agent_list_profits', array('' => 'Select agent')+$agent_list, '', 'class=form-control id=agent_list_profits'); ?>
                        </label>
                    </div>
                    <div class="col-md-3">
                        <label>
                            <select id="subagent_list_profits" name="subagent_list_profits" class="form-control form_control_custom">
                                <option value="" selected="selected">Select a Subagents</option>                                
                            </select>
                        </label>
                    </div>
                   <div class="col-md-3">
                        <label>
                                <select id="admin_list_profits" name="admin_list_profits" class="form-control form_control_custom">
